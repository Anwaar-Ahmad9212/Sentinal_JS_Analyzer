      ...
      AssignmentExpression(path: any) {
        ...
        if (xssSinks.includes(sinkName) || String(sinkName).startsWith('on')) {
          ...
          if (dangerous && !sanitized) {
            const rightCode = code.substring(node.right.start, node.right.end);
            issues.push({
              type: VulnerabilityType.XSS,
              ...
              explanation: `Untrusted user input (${rightCode}) flows into a dangerous DOM sink (${sinkName}) without proper sanitization. This allows an attacker to execute arbitrary JavaScript in the victim's browser context.`,
              fix_steps: [
              ...
              fix_code: `DOMPurify.sanitize(${rightCode})`,
           ...
           node.arguments.forEach((arg: any) => {
             ...
             if (dangerous && !sanitized) {
               ...
               issues.push({
                 ...
                 explanation: `The application renders untrusted user input (${argCode}) directly into the response via ${fnName}(), which can lead to Cross-Site Scripting.`,
                 fix_steps: [
                   "Escape all user-provided data before rendering.",
                   "Set appropriate Content-Type headers (e.g., text/plain if not HTML).",
                   "Implement a strict Content Security Policy (CSP)."
                 ],
                 fix_code: `escapeHtml(${argCode})`,
